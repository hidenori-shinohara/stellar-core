...
CommandHandler::CommandHandler(Application& app) : mApp(app)
{
    ...
    addRoute("archive-report", &CommandHandler::archiveReport);
...
void
CommandHandler::archiveReport(std::string const&, std::string& retStr)
{
    ZoneScoped;
    mApp.getHistoryArchiveManager().logReportOnArchiveConfig();
}
